...
CHALLENGE_LEVEL = random.randint(20, 40)
...
class Node:
    ...
    def search(transaction_id, challenge, pid, conn_send, speed):
        chalenge_str = "0b" + "".join(["0"] * challenge)
        N_ceil = math.ceil(challenge / 8)

        seed = "".join(
            (text_to_bits(random.choice(SEED_VALID)) for _ in range(challenge))
        )
        hash_object = bin(int(hashlib.sha1(seed.encode()).hexdigest(), 16))[
            : N_ceil + 2
        ]

        while not hash_object.startswith(chalenge_str):
            seed = "".join(
                [text_to_bits(random.choice(SEED_VALID)) for _ in range(challenge)]
            )
            hash_object = bin(int(hashlib.sha1(seed.encode()).hexdigest(), 16))[
        ...
        seed_str = "".join(
            (text_from_bits("".join(s)) for s in zip(*(iter(seed),) * 8))
        )
        conn_send.send(
    ...
    def verify(self, node_id, transaction_id, seed):
        ...
        seed_ = "".join((text_to_bits(s) for s in seed))
        hash_object = bin(int(hashlib.sha1(seed_.encode()).hexdigest(), 16))[
            : N_ceil + 2
        ]
